...
async function analyzeDependencies(): Promise<string[]> {
  ...
  // Moduli built-in di Node.ts che NON devono essere installati come dipendenze
    ...
    for (const file of serverFiles) {
      ...
      while ((match = importRegex.exec(content)) !== null) {
        ...
        if (!dep.startsWith('.') && !dep.startsWith('node:') && !dep.startsWith('/')) {
          ...
          // Escludi i moduli built-in di Node.ts
...
async function getPackageDependencies(pkgPath: string): Promise<string[]> {
  try {
...
async function copyNodeModules(dependencies: string[]): Promise<void> {
  ...
  let copied = 0;
  let notFound = 0;
  const allPackages = new Set<string>(dependencies);
  const processed = new Set<string>();

  // Funzione ricorsiva per copiare pacchetto e le sue dipendenze
  async function copyPackage(pkg: string): Promise<void> {
...
async function addToFunctionsPackageJson(dependencies: string[]): Promise<void> {
  console.log('\n📝 Aggiornamento functions/package.tson...');

  const functionsPackageJsonPath = path.join(rootDir, 'functions', 'package.json');
  const rootPackageJsonPath = path.join(rootDir, 'package.json');

  try {
    // Leggi il package.tson del progetto root per ottenere le versioni
    ...
    // Leggi il package.tson di functions
    ...
    for (const dep of dependencies) {
      if (!functionsPkg.dependencies[dep]) {
        // Prendi la versione dal root package.tson se disponibile
    ...
    if (added > 0) {
      // Scrivi il package.tson aggiornato
      await fs.writeFile(
        functionsPackageJsonPath,
        JSON.stringify(functionsPkg, null, 2) + '\n'
      );
      console.log(`\n✅ Aggiunte ${added} dipendenze a functions/package.tson`);
      console.log('\n⚠️  IMPORTANTE: Esegui "cd functions && npm install" prima del deploy!');
    } else {
      console.log('✅ Tutte le dipendenze sono già presenti in functions/package.tson');
    }
  } catch (error) {
